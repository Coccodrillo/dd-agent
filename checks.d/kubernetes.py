...
class Kubernetes(AgentCheck):
    ...
    def _process_events(self, instance, pods_list):
        ...
        involvedObject.name=<pod>,involvedObject.uid=<pod_uid>
        """
        ...
        events_endpoint = urljoin(self.kubeutil.kubelet_api_url, k8s_namespace, 'events')
        self.log.debug('Endpoint to query events:', events_endpoint)
        events = retrieve_json(events_endpoint)
        ...
        for event in event_items:
            ...
            if involved_obj and involved_obj.get('uid') in pod_uids:
                self.log.warning('Should post this event:', event.get('reason'), event.get('message'))